
import base64
from typing import List
from urllib import response
import uuid
import modal
import os
import boto3
# import boto3
from pydantic import BaseModel
import requests


from prompts import LYRICS_GENERATOR_PROMPT, PROMPT_GENERATOR_PROMPT


app = modal.App("music-generator")

image = (
    modal.Image.debian_slim()
    .apt_install("git", "ffmpeg","libsndfile1")
    .pip_install_from_requirements("requirements.txt")
    .run_commands(["git clone https://github.com/ace-step/ACE-Step.git /tmp/ACE-Step", "cd /tmp/ACE-Step && pip install ."])
    .env({"HF_HOME": "/.cache/huggingface"})
    .add_local_python_source("prompts")
)

model_volume = modal.Volume.from_name(
    "ace-step-models", create_if_missing=True)
hf_volume = modal.Volume.from_name("qwen-hf-cache", create_if_missing=True)

music_gen_secrets = modal.Secret.from_name("music-gen-secret")

# thsi is the function that will be run in the cloud @modal gpu
# @app.function(image = image ,secrets=[modal.Secret.from_name("music-gen-secret")])
# def function_test():
#     print("Hello, World!")
#     print(os.environ["test"])
# it was jist for the testing 

class AudioGenerationBase(BaseModel):
    audio_duration: float = 180.0  # in seconds
    seed:int = -1
    guidance_scale: float = 15.0
    infer_step: int = 60
    instrumental: bool = False


class GenerateFromDescriptionRequest(AudioGenerationBase):
    full_described_song: str
class GenerateFromCustomLyricsRequest(AudioGenerationBase):
    prompt: str
    lyrics: str
class GenerateWithDescribedLyricsRequest(AudioGenerationBase):
    prompt: str
    described_lyrics: str
    
    
class GenerateMusicResponseS3(BaseModel):
    audio_S3_key: str
    cover_image_s3_key:str
    categories: List[str]


class GenerateMusicResponse(BaseModel):
    audio_data : str  # base64 encoded audio data

    
@app.cls(
    image=image,
    gpu="L40S",
    volumes={"/models": model_volume, "/.cache/huggingface": hf_volume},
    secrets=[music_gen_secrets],
    scaledown_window=15
) 
class MusicGenServer:
    
    @modal.enter()
    def lode_model(self):
        from acestep.pipeline_ace_step import ACEStepPipeline
        from transformers import AutoModelForCausalLM, AutoTokenizer
        from diffusers import AutoPipelineForText2Image
        import torch
        import uuid
        # Musicx generation model
        self.music_model = ACEStepPipeline(
            checkpoint_dir="/models",
            dtype="bfloat16",
            torch_compile=False,
            cpu_offload=False,
            overlapped_decode=False
        )
        
        # LLM model for prompt generation
        model_id = "Qwen/Qwen2-7B-Instruct"
        self.tokenizer = AutoTokenizer.from_pretrained(model_id)

        self.llm_model = AutoModelForCausalLM.from_pretrained(
            model_id,
            torch_dtype="auto",
            device_map="auto",
            cache_dir="/.cache/huggingface"
        )
        
        # Stable Diffusion model for cover image generation
        self.image_pipe = AutoPipelineForText2Image.from_pretrained(
            "stabilityai/sdxl-turbo",
            torch_dtype=torch.float16,
            variant="fp16",
            cache_dir="/.cache/huggingface")
        self.image_pipe.to("cuda")
        
    def prompt_qwen(self, question:str ):
        messages = [
            {"role": "user", "content": question}
        ]
        text = self.tokenizer.apply_chat_template(
            messages,
            tokenize=False,
            add_generation_prompt=True
        )
        model_inputs = self.tokenizer([text], return_tensors="pt").to(self.llm_model.device)

        generated_ids = self.llm_model.generate(
            model_inputs.input_ids,
            max_new_tokens=512
        )
        generated_ids = [
            output_ids[len(input_ids):] for input_ids, output_ids in zip(model_inputs.input_ids, generated_ids)
        ]

        response = self.tokenizer.batch_decode(generated_ids, skip_special_tokens=True)[0]
        
        return response

    # def generate_prompt (self, description: str):
    #     # Format the prompt for the LLM
    #     full_prompt = PROMPT_GENERATOR_PROMPT.format(user_prompt=description)
        
    #     # Call the LLM to generate the prompt
    #     return self.prompt_qwen(full_prompt)
    def generate_prompt(self, description: str):
        full_prompt = PROMPT_GENERATOR_PROMPT.format(user_prompt=description)
        return self.prompt_qwen(full_prompt)

    def generate_lyrics(self, description: str):
        full_prompt = LYRICS_GENERATOR_PROMPT.format(description=description)
        return self.prompt_qwen(full_prompt)

    
    # def generate_lyrics (self, description: str):
    #     # Format the prompt for the LLM
    #     full_prompt = LYRICS_GENERATOR_PROMPT.format(user_prompt=description)
        
    #     # Call the LLM to generate the prompt
    #     return self.prompt_qwen(full_prompt)
    
    def generate_categories(self, description: str) -> List[str]:
        # Placeholder implementation for category generation
        prompt = f"Based on the following music description, list 3-5 relevant genres or categories as a comma-separated list. For example: Pop, Electronic, Sad, 80s. Description: '{description}'"

        response_text   = self.prompt_qwen(prompt)
        categories = [cat.strip() for cat in response_text.split(",") if cat.strip()]
        
        # In a real scenario, this could involve another model or a predefined mapping
        # Example categories
        return categories
        
    def generate_and_uplode_to_s3(
        self,
        prompt: str,
        lyrics: str,
        instrumental: bool,
        audio_duration: float,
        infer_step: int,
        guidance_scale: float,
        seed: int,
        description_for_categorization: str,
    ) -> GenerateMusicResponseS3:
        
        final_lyrics = "[instrumental]" if instrumental else lyrics
        print(f"generating lyrics \n{final_lyrics}")
        print(f"generating prompt \n{prompt}")
        
        
        # AWS S3 UPLOAD LOGIC TO BE IMPLEMENTED HERE
        # will store the lirics prompt the generated music and the cover image to S3
        
        # IAM user :
            # - Backend :
            # will have access to upload to the S3 bucket
            #   PutObject permissions
            # - Frontend (next-js) will have access to upload only to a 
            # specific folder/prefix in the S3 bucket\
            #  GetObject , List permissions
        
        s3_client = boto3.client('s3')
        bucket_name = os.environ["S3_BUCKET_NAME"]# replace with your bucket
        
        output_dir = "/tmp/outputs"
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, f"{uuid.uuid4()}.wav")

        self.music_model(
            prompt=prompt,
            lyrics=final_lyrics,
            audio_duration=audio_duration,
            infer_step=infer_step,
            guidance_scale=guidance_scale,
            save_path=output_path,
            manual_seeds=str(seed)
        )
        
        audio_s3_key = f"{uuid.uuid4()}.wav"
        s3_client.upload_file(output_path, bucket_name, audio_s3_key)  
        os.remove(output_path)
        
        # Generate cover image
        
        thumbnail_PROMPT = f"An album cover image for a music track with the theme: {prompt}. Highly detailed, vibrant colors, artistic style."
        image = self.image_pipe(prompt=thumbnail_PROMPT, num_inference_step = 2 , guidance_scale= 0.0).images[0]
        image_output_path = os.path.join(output_dir, f"{uuid.uuid4()}.png")
        image.save(image_output_path) 
        
        cover_image_s3_key = f"{uuid.uuid4()}.png"
        s3_client.upload_file(image_output_path, bucket_name, cover_image_s3_key)
        os.remove(image_output_path)
        
        # catagory generation
        
        categories = self.generate_categories(description_for_categorization)
        
        return GenerateMusicResponseS3(
            audio_S3_key=audio_s3_key,
            cover_image_s3_key=cover_image_s3_key,
            categories=categories
        )
    
    @modal.fastapi_endpoint(method="POST")
    def generate(self) -> GenerateMusicResponse:
        output_dir = "/tmp/outputs"
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, f"{uuid.uuid4()}.wav")

        self.music_model(
            prompt="electronic rap",
            lyrics="[verse]\nWaves on the bass, pulsing in the speakers,\nTurn the dial up, we chasing six-figure features,\nGrinding on the beats, codes in the creases,\nDigital hustler, midnight in sneakers.\n\n[chorus]\nElectro vibes, hearts beat with the hum,\nUrban legends ride, we ain't ever numb,\nCircuits sparking live, tapping on the drum,\nLiving on the edge, never succumb.\n\n[verse]\nSynthesizers blaze, city lights a glow,\nRhythm in the haze, moving with the flow,\nSwagger on stage, energy to blow,\nFrom the blocks to the booth, you already know.\n\n[bridge]\nNight's electric, streets full of dreams,\nBass hits collective, bursting at seams,\nHustle perspective, all in the schemes,\nRise and reflective, ain't no in-betweens.\n\n[verse]\nVibin' with the crew, sync in the wire,\nGot the dance moves, fire in the attire,\nRhythm and blues, soul's our supplier,\nRun the digital zoo, higher and higher.\n\n[chorus]\nElectro vibes, hearts beat with the hum,\nUrban legends ride, we ain't ever numb,\nCircuits sparking live, tapping on the drum,\nLiving on the edge, never succumb.",
            audio_duration=180,
            infer_step=60,
            guidance_scale=15,
            save_path=output_path,
        )
        
        with open(output_path, "rb") as f:
            audio_bytes = f.read()

        audio_b64 = base64.b64encode(audio_bytes).decode("utf-8")

        os.remove(output_path)

        return GenerateMusicResponse(audio_data=audio_b64)
    
    # @modal.fastapi_endpoint(method="POST")
    # def generate_from_description(self, request:GenerateFromDescriptionRequest) -> GenerateMusicResponseS3:
        
    #     # Generate Prompt using the description
    #     prompt = self.generate_prompt(request.full_described_song)
        
    #     # Generate Lyrics  for the song based on description
    #     lyrics = ""
    #     if not request.instrumental:
    #         lyrics = self.generate_lyrics(request.full_described_song)
            
    #     return self.generate_and_uplode_to_s3(
    #         prompt=prompt,
    #         lyrics=lyrics,
    #         description_for_categorization=request.full_described_song,
    #         **request.model_dump(exclude={"full_described_song"})
    #     )
    
    
    @modal.fastapi_endpoint(method="POST")
    def generate_from_description(self, request: GenerateFromDescriptionRequest) -> GenerateMusicResponseS3:
        # Generating a prompt
        prompt = self.generate_prompt(request.full_described_song)

        # Generating lyrics
        lyrics = ""
        if not request.instrumental:
            lyrics = self.generate_lyrics(request.full_described_song)
        return self.generate_and_uplode_to_s3(prompt=prompt, lyrics=lyrics,
                                              description_for_categorization=request.full_described_song, **request.model_dump(exclude={"full_described_song"}))
    
    @modal.fastapi_endpoint(method="POST")
    def generate_with_lyrics(self, request:GenerateFromCustomLyricsRequest) -> GenerateMusicResponseS3:
        
        # generate prompt for the thumbnail image
        
        # generate music 
        return self.generate_and_uplode_to_s3(
            prompt=request.prompt,
            lyrics=request.lyrics,
            description_for_categorization=request.prompt,
            **request.model_dump(exclude={""})
        )
    
    @modal.fastapi_endpoint(method="POST")
    def generate_with_described_lyrics(self, request: GenerateWithDescribedLyricsRequest) -> GenerateMusicResponseS3:
        
        # generate prompt for the thumbnail imag
        
        lyrics = ""
        if not request.instrumental:
            lyrics = self.generate_lyrics(request.described_lyrics)
            
        return self.generate_and_uplode_to_s3(
            prompt=request.prompt,
            lyrics=lyrics,
            description_for_categorization=request.prompt,
            **request.model_dump(exclude={"described_lyrics"})
        )
    


@app.local_entrypoint()
def main():
    
    server = MusicGenServer()
    print("server loaded")
    
    endpoint_url = server.generate_from_description.get_web_url()
    print(f"Endpoint URL: {endpoint_url}")
    
    request_data = GenerateFromDescriptionRequest(
        full_described_song="rain songs based on adventure ",
        guidance_scale=12.5,
    )
    
    payload = request_data.model_dump()
    
    response = requests.post(
        endpoint_url,
        json=payload
    )
    response.raise_for_status()  # just call it, don’t assign it
    result = GenerateMusicResponseS3.model_validate(**response.json())
    
    print("Audio S3 Key:", result.audio_S3_key)
    print("Cover Image S3 Key:", result.cover_image_s3_key)
    print("Categories:", result.categories)
    
    print ("Music generation request completed.")
